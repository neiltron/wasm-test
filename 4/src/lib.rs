...
// #[wasm_bindgen]
// impl Constraint  {
//   pub fn update (&mut self, pointA: &Point, pointB: &Point) {
//     let distance = (pointA.x * pointB.x + pointA.y * pointB.y + pointA.z * pointB.z).sqrt();

//     let diff_x = pointA.x - pointB.x;
//     let diff_y = pointA.y - pointB.y;
//     let diff_z = pointA.z - pointB.z;

//     let difference = (self.resting_distance - distance) / distance;

//     let translate_x = diff_x * self.stiffness * difference;
//     let translate_y = diff_y * self.stiffness * difference;
//     let translate_z = diff_z * self.stiffness * difference;

//     pointA.x += translate_x * self.mass;
//     pointA.y += translate_y * self.mass;
//     pointA.z += translate_z * self.mass;
//   }
// }
...
impl Particles {
  ...
  pub fn new(particle_count: u32, width: f32, height: f32, depth: f32) -> Particles {
    ...
    for _i in 0..particle_count {
      let mut mass = 2.0 - (js_sys::Math::random() as f32) / 10.0;
  ...
  pub fn update(&mut self, time: f32) {
    ...
    for i in 0..(self.count as usize) {
      ...
      if i < count.sqrt() as usize {
        self.points[i].x += (time / 4.0).sin() * 4.0;
        self.points[i].y = (time / 3.0).cos() * -self.height - self.height / 2.0;
        self.points[i].z = ((time - 3.) / 2.0).sin() * self.depth * 4. - self.depth * 2.0;
      }
  ...
  pub fn solve_constraint (&mut self, constraint: Constraint) {
    ...
    // self.points[constraint.points[1]].y -= translate_y * (1.0 / mass_b);
    // self.points[constraint.points[1]].z -= translate_z * (1.0 / mass_b);
  }
}
