...
impl Particles {
  ...
  pub fn new(particle_count: u32, width: f32, height: f32, radius: f32) -> Particles {
    ...
    for _i in 0..particle_count {
      let point = Point {
        x: width / 2.0 + 100. * ((js_sys::Math::random() as f32) - 0.5),
        y: height / 2.0 + 100. * ((js_sys::Math::random() as f32) - 0.5),
  ...
  pub fn update(&mut self) {
    ...
    for i in 0..(self.count as usize) {
      ...
      self.vertices[i * 2] = self.points[i].x;
      self.vertices[i * 2 + 1] = self.points[i].y;

      self.points[i].speed = self.points[i].speed.lerp(0.005, 0.05);
    }
  }
